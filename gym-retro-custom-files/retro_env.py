...
class RetroEnv(gym.Env):
    ...
    def __init__(self, game, state=retro.State.DEFAULT, scenario=None, info=None, use_restricted_actions=retro.Actions.FILTERED,
        ...
        if state == retro.State.NONE:
            ...
            try:
                ...
                ## Custom Function only applies to SMB Nes
                if self.gamename == 'SuperMarioBros-Nes':
                    self.random_state = True
                    self.stage_rng = np.random.default_rng()
                    world = self.stage_rng.integers(1, 8, endpoint=True)
                    self.statename = f"Level{world}-1.state"
                    # print(f"Init: Selected World {world}-1")
                else:
                    self.statename = None
    ...
    def reset(self):
        if self.initial_state:
            ## Custom Function only applies to SMB Nes
            if self.gamename == 'SuperMarioBros-Nes' and self.random_state:
                world = self.stage_rng.integers(1, 8, endpoint=True)
                statename = f"Level{world}-1.state"
                
                if not statename.endswith('.state'):
                    statename += '.state'
                with gzip.open(retro.data.get_file_path(self.gamename, statename, inttype=retro.data.Integrations.DEFAULT), 'rb') as fh:
                    self.initial_state = fh.read()
                # print(f"Reset: Selected World {world}-1")
